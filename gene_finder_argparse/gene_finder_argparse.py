...
def gene_finder(gff_file, gene_wanted): # la fonction prend en arguments le fichier gff avec les annotations (le GCA) et le gène recherché (attention à l'orthographe)
    ...
    with open(gff_file, 'r') as gff : # on ouvre le fichier gff en question (on le renomme "gff" juste dans la fonction pour faciliter l'écriture)
         for line in gff: # Pour chaque ligne du fichier, on effectue la commande en dessous
            if not line.startswith("#") : # Si la ligne ne commence PAS par un #...
                ...
                second_part = re.split(r'[ =\t:;()0-9]+', qualifiers) # définit la "seconde partie" de la ligne, et divise uniquement cette dernière (grâce à qualifiers) selon les séparateurs qui sont l'espace, =, la tabulation, :, ;, (), et les nombres de 0 à 9. Le "+" indique qu'il peut y avoir plusieurs de chacun de ces séparateurs
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Use a GFF file with annotations to find the wanted gene')
    parser.add_argument('gene_wanted', type=str, help='the gene that you are looking for')
    parser.add_argument('gff_file', type=str,help='gff file with the annotations')

    args = parser.parse_args() # analyse et stockage des valeurs entrées dans args 
    gene_wanted = args.gene_wanted # on associe la valeur entrée à gene_wanted 
    gff_file = args.gff_file # on associe les valeurs des arguments à gff_file

    genes, number = gene_finder(gff_file, gene_wanted)
